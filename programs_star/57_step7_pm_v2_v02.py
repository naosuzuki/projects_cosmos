...
def build_pm_table(combo_name, m1, m2, tag1, tag2, ep1, ep2, cw_agn):
    ...
    int_df = pd.read_parquet(OUT / f'refined_{combo_name}_v02.parquet')
    base = pd.DataFrame({
...
def process_combo(args) -> dict:
    ...
    out_parq = OUT / f'refined_{combo_name}_v02_with_pm_clean.parquet'
    out.to_parquet(out_parq, index=False)
    csv_written = False
    if len(out) * len(out.columns) < 200_000_000:
        out.to_csv(OUT / f'refined_{combo_name}_v02_with_pm_clean.csv', index=False)
        csv_written = True
