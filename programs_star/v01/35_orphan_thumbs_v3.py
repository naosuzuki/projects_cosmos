...
def main():
    ...
    # ---------- 1. Top orphans v3 (Gaia/peak/local-veto applied) ----------
    orph = pd.read_parquet(OUT / 'orphans_highconf_v3.parquet')
    orph_top = (
        orph.sort_values('snr', ascending=False)
            .groupby('found_in').head(TOP_N_PER_MISSION)
            .reset_index(drop=True)
    )
    orph_top['kind'] = 'v3_' + orph_top['found_in'].str.replace(' ', '_')
    print(f'orphans v3 → top {len(orph_top):,}')
    ...
    html = (
        ...
        '<title>COSMOS star v01 — orphan + high-PM thumbnails (v3)</title>'
        ...
        '</head><body>'
        '<h1>COSMOS star catalog v01 — diagnostic thumbnails (v3)</h1>'
        '<p>v3 = bright-star contaminants removed: mag-scaled Gaia veto (G&lt;14:30″, G&lt;16:15″, G&lt;18:5″, G&lt;21:2″) + peak above per-band 99th-pctl + local bright neighbour (peak above 99.5-pctl within 30″).</p>'
