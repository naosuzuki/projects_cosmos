#!/usr/bin/env python
"""
65_sed_blackbody_v05.py — SED v05: global blackbody fit EXCLUDING Euclid,
then predict Euclid and test it.

Same star sample as v04 (pure stars; AGN/QSO excluded, photometry kept).
Like v04 the fit has no HST+JWST anchor — BUT the four Euclid bands (VIS,
NISP Y/J/H) are left OUT of the fit.  The blackbody is constrained by
every OTHER facility (HST F814W + JWST NIRCam + CFHT u + Subaru HSC
g/r/i/z/y + Spitzer IRAC ch1-4 + JWST MIRI F770W).  We then predict the
Euclid magnitudes and measure Δ = observed − predicted to test how well
Euclid matches the all-facility consensus continuum.

Validity (same as v04): flux>0, flux_err<1e10, SNR>3, and (for COSMOS-Web
bands) flux not bit-identical across ≥2 bands (per-source fill).

Outputs:
  csvfiles_star/sed_blackbody_v05.parquet (+ .csv)
  htmls/sed_v05/band_offset_summary.png/.txt
  htmls/sed_v05/seds/sed_<id>.png
  htmls/sed_v05/index.html  (paginated)
"""
...
# v05: Euclid bands are TEST-only — excluded from the blackbody fit.
EUCLID_KEYS = {'Euclid_VIS', 'Euclid_Y', 'Euclid_J', 'Euclid_H'}
IN_FIT = {b[0]: (b[0] not in EUCLID_KEYS) for b in BANDS}
...
def main(validate_only=False):
    ...
    for _, row in df.iterrows():
        ...
        # v05: fit to all valid bands EXCEPT Euclid (Euclid is test-only)
        fit_keys = [k for k in all_keys if valid[k] and IN_FIT[k]]
        if len(fit_keys) < MIN_FIT_BANDS:
            continue
        # GLOBAL fit to all valid NON-Euclid bands
    ...
    res.to_parquet(OUT/'sed_blackbody_v05.parquet', index=False)
    res.to_csv(OUT/'sed_blackbody_v05.csv', index=False)
    print(f'Wrote sed_blackbody_v05.parquet ({len(res):,} rows; photometry kept for all)')
...
def make_sed_plot(rec, filt, shape_grid, path):
    ...
    def marker(k):
        if k in EUCLID_KEYS: return 'D'   # Euclid = test (not in fit)
        return 'o' if SRC[k]=='ours' else 's'
    ...
    handles=[]
    for k in order:
        src = ('Euclid TEST' if k in EUCLID_KEYS else ('ours' if SRC[k]=='ours' else MIS[k]))
    ...
    ax.set_title(f'{int(rec["id"])}  —  T={T:.0f}K, χ²ν={rec["chi2_red"]:.2f}  '
                 f'(fit excl. Euclid, {int(rec["n_fit_bands"])} bands)')
    ax.grid(alpha=0.3, which='both')
    fig.tight_layout(); fig.savefig(path, dpi=110); plt.close(fig)


def summarize(res, filt):
    lines = ['SED v05 offset test (observed - blackbody-predicted; Euclid EXCLUDED from fit)',
             'Blackbody fit to HST+JWST+CFHT+HSC+IRAC+MIRI; Euclid VIS/Y/J/H predicted & tested.', '']
    ...
    for key, mission, source, piv, med, nmad, n in rows:
        mk = 'D' if key in EUCLID_KEYS else ('o' if source=='ours' else 's')
    ...
    ax.set_title('SED v05 — Euclid (diamond) tested vs blackbody fit to all OTHER facilities')
    from matplotlib.lines import Line2D
...
def build_webform(res_plotted, res_all, per_page=60, prefix='index', banner=None):
    ...
    for pg in range(n_pages):
        ...
        nav=' '.join(parts)
        html=f'''<!doctype html><html><head><meta charset="utf-8"><title>SED v05 {prefix} — page {pg+1}/{n_pages}</title>
...
<h1>SED v05 — Euclid tested vs blackbody fit to all other facilities
...
{bnr}<div class="summary">{off}</div><div class="nav">{nav}</div><div class="grid">{''.join(cards)}</div><div class="nav">{nav}</div>
    ...
    print(f'Webform ({prefix}): {n_pages} pages → htmls/sed_v05/{prefix}.html')
