...
def build_training_set(cw_flags: pd.DataFrame) -> pd.DataFrame:
    ...
    df = pd.read_parquet(OUT / 'cross_HST_JWST_Euclid_VIS_Euclid_NISP_v03.parquet')
...
def apply_pointsource_flags(combo_name: str, missions: list[str],
    ...
    df = pd.read_parquet(OUT / f'cross_{combo_name}_v03.parquet')
    n_total = len(df)
    ...
    # Write
    out_parq = OUT / f'refined_{combo_name}_v03.parquet'
    df.to_parquet(out_parq, index=False)
    csv_written = False
    if len(df) * len(df.columns) < 200_000_000:
        df.to_csv(OUT / f'refined_{combo_name}_v03.csv', index=False)
        csv_written = True
