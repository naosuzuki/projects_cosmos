#!/usr/bin/env python
"""
60_master_or_catalog_v03.py — STEP 8 v02 master OR catalog with bug fixes.

Bug fixes vs v01 (documented in programs_star/CLAUDE.md):
  B1 — primary_id now uniquely-keyed (HST_v03 has globally-unique hst_id)
...
Inputs: cat_matched_*_with_gaia_v03.parquet (v02 fixed-id HST),
        refined_*_with_pm_v03.parquet (legacy per-tile ids; matched via
        acs_number+tile join).

Output: csvfiles_star/master_or_catalog_v03.parquet (+.csv if small).
"""
...
def main():
    ...
    for m in MISSIONS:
        ...
        df = pd.read_parquet(OUT / f'cat_matched_{m}_with_gaia_v03.parquet')
    ...
    for pair in PM_PAIRS:
        pm_path = OUT / f'refined_{pair}_v03_with_pm_clean.parquet'
        ...
        if 'JWST' in pair:
            ...
            # is the globally-unique idx_c (from 52_v03) propagated through
        ...
        if False:  # OBSOLETE block (kept dead for diff readability) — old (acs,tile) join:
            ...
            # hst_id_hst alone. refined_*_with_pm_v03 uses legacy per-tile
    ...
    cat_mag_F115   = _arr('cat_mag_F115W_jwst')
    cat_mag_F150   = _arr('cat_mag_F150W_jwst')
    cat_mag_F277   = _arr('cat_mag_F277W_jwst')
    cat_mag_F444   = _arr('cat_mag_F444W_jwst')
    cat_mag_Y      = _arr('cat_mag_NIR_Y_nisp')
    cat_mag_J      = _arr('cat_mag_NIR_J_nisp')
    cat_mag_H      = _arr('cat_mag_NIR_H_nisp')
    cat_class_star = _arr('cat_class_star_hst')
    cat_mu_class   = _arr('cat_mu_class_hst')
    has_hst_cat = ~np.isnan(_arr('hst_id'))

    # v03 NEW: per-band saturation flags using empirically-verified thresholds
    # (spot-checked against real HST/JWST/Euclid imagery 2026-05-27).
    #   HST F814W:     saturated < 20,  likely_saturated < 21
    #   JWST any band: saturated < 20,  likely_saturated < 21
    #   Euclid VIS:    saturated < 19,  likely_saturated < 20
    #   Euclid NISP:   saturated < 15,  likely_saturated < 16
    sat_hst   = _mask_lt(cat_mag_F814, 20.0)
    lk_hst    = _mask_lt(cat_mag_F814, 21.0)
    sat_jwst  = (_mask_lt(cat_mag_F115, 20.0) | _mask_lt(cat_mag_F150, 20.0)
                  | _mask_lt(cat_mag_F277, 20.0) | _mask_lt(cat_mag_F444, 20.0))
    lk_jwst   = (_mask_lt(cat_mag_F115, 21.0) | _mask_lt(cat_mag_F150, 21.0)
                  | _mask_lt(cat_mag_F277, 21.0) | _mask_lt(cat_mag_F444, 21.0))
    sat_vis   = _mask_lt(cat_mag_VIS, 19.0)
    lk_vis    = _mask_lt(cat_mag_VIS, 20.0)
    sat_nisp  = (_mask_lt(cat_mag_Y, 15.0) | _mask_lt(cat_mag_J, 15.0) | _mask_lt(cat_mag_H, 15.0))
    lk_nisp   = (_mask_lt(cat_mag_Y, 16.0) | _mask_lt(cat_mag_J, 16.0) | _mask_lt(cat_mag_H, 16.0))
    saturated_any        = sat_hst | sat_jwst | sat_vis | sat_nisp
    likely_saturated_any = lk_hst  | lk_jwst  | lk_vis  | lk_nisp
    df_out['saturated_hst']         = sat_hst
    df_out['saturated_jwst']        = sat_jwst
    df_out['saturated_vis']         = sat_vis
    df_out['saturated_nisp']        = sat_nisp
    df_out['saturated_any']         = saturated_any
    df_out['likely_saturated_any']  = likely_saturated_any
    ...
    # NOTE: saturation flags (saturated_any, likely_saturated_any) are
    # kept as separate columns but DELIBERATELY NOT in is_likely_star.
    # The +1/+2 mag thresholds (e.g. JWST mag<20) catch bright SNe at peak
    # too: tested SN 63919/78323 (JWST-discovered) had saturated_any=True
    # because they're bright NIR sources. True stars at those brightnesses
    # are already caught via Gaia / cat_class_star / cat_phz_classification.
    is_likely_star = (
        gaia_present
    ...
    meta = ['primary_id', 'primary_source',
            ...
            'saturated_any', 'likely_saturated_any',
            'saturated_hst', 'saturated_jwst', 'saturated_vis', 'saturated_nisp']
    ...
    out_parq = OUT / 'master_or_catalog_v03.parquet'
    df_out.to_parquet(out_parq, index=False)
    if len(df_out) * len(df_out.columns) < 300_000_000:
        df_out.to_csv(OUT / 'master_or_catalog_v03.csv', index=False)
