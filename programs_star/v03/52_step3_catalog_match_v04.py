#!/usr/bin/env python
"""
52_step3_catalog_match_v02.py — STEP 3 v02 (fixes bug B1).

Bug B1 (programs_star/CLAUDE.md): the ACS i-phot catalog's `number`
column is per-tile SExtractor numbering, NOT globally unique
(1,177,274 rows but only 3,538 unique `number` values, max 775
duplicates). Using cat_num as hst_id propagated to massive primary_id
collisions downstream (29.4× duplication in master_or_catalog_v01).

v02 fix: set `hst_id = idx_c` (catalog row index — globally unique
by construction). Preserve the legacy per-tile `cat_num` as a new
column `acs_number` for traceability and as the join key against
older intermediates that still use it.

JWST cw_id and Euclid object_id are already globally unique;
their v02 outputs are functionally identical to v01, just renamed.

Outputs:
  csvfiles_star/cat_matched_HST_v02.parquet         + .csv
  csvfiles_star/cat_matched_JWST_v02.parquet        + .csv
  csvfiles_star/cat_matched_Euclid_VIS_v02.parquet  + .csv
  csvfiles_star/cat_matched_Euclid_NISP_v02.parquet + .csv
"""
...
def step3_hst():
    ...
    out_cat = pd.DataFrame({
        # v02 fix B1: hst_id = catalog row index (globally unique).
        # The legacy per-tile cat_num is preserved as `acs_number`.
        'hst_id'              : idx_c.astype(np.int64),
        'acs_number'          : cat_num[idx_c].astype(np.int64),
        'cat_ra'              : cat_ra[idx_c],
        ...
        'hst_source_type'     : 'cat_matched',
    })
    out_cat = pd.concat([out_cat, d], axis=1)
    print(f'  catalog-matched DAO entries: {len(out_cat):,}')

    # v03 fix B3 root: keep unmatched bright DAO detections as supplements.
    # Saturated stars are dropped by the published ACS catalog (mask)
    # but DAO still finds their peaks. Include them so cross-mission
    # matching (54_v03) can pair them with their JWST/Euclid counterparts,
    # then PM can be measured. Threshold snr>100 keeps ~130k bright
    # candidates (mostly saturated stars) and excludes faint noise peaks.
    SNR_SUPPL_THRESHOLD = 100.0
    matched_mask = np.zeros(len(dao), dtype=bool)
    matched_mask[idx_d] = True
    suppl = dao.loc[(~matched_mask) & (dao['snr'] > SNR_SUPPL_THRESHOLD)].copy()
    n_suppl = len(suppl)
    print(f'  unmatched bright (snr>{SNR_SUPPL_THRESHOLD:.0f}) DAO supplements: {n_suppl:,}')
    if n_suppl:
        # Synthetic hst_id starting at 10_000_000 to never collide with
        # catalog row indices (which top out at ~1.18M).
        ID_OFFSET = 10_000_000
        suppl = suppl.reset_index(drop=True)
        d_suppl = suppl[DAO_COLS_KEEP + ['snr']].add_prefix('dao_F814W_')
        out_suppl = pd.DataFrame({
            'hst_id'              : np.arange(len(suppl), dtype=np.int64) + ID_OFFSET,
            'acs_number'          : np.full(len(suppl), -1, dtype=np.int64),  # no ACS entry
            'cat_ra'              : suppl['ra'].values,    # use DAO position
            'cat_dec'             : suppl['dec'].values,
            'cat_mag_F814W'       : np.nan,
            'cat_magerr_F814W'    : np.nan,
            'cat_class_star'      : np.nan,
            'cat_mu_class'        : np.nan,
            'cat_flags'           : np.nan,
            'cat_fwhm_image_as'   : np.nan,
            'cat_a_image_as'      : np.nan,
            'cat_b_image_as'      : np.nan,
            'cat_sep_arcsec_F814W': np.nan,
            'hst_source_type'     : 'dao_only_bright',
        })
        out_suppl = pd.concat([out_suppl, d_suppl], axis=1)
        out = pd.concat([out_cat, out_suppl], ignore_index=True)
    else:
        out = out_cat

    p = OUT / 'cat_matched_HST_v04.parquet'
    out.to_parquet(p, index=False)
    out.to_csv(p.with_suffix('.csv'), index=False)
    print(f'  → {p}  ({len(out):,} rows total: '
          f'{int((out["hst_source_type"]=="cat_matched").sum()):,} cat-matched + '
          f'{int((out["hst_source_type"]=="dao_only_bright").sum()):,} dao-only supplements)')
...
def step3_jwst():
    ...
    # ── v04 COMPLETENESS FIX (2026-06-01) ──────────────────────────────
    # v03 kept ONLY catalog ids with a DAO peak within JWST_PIX_AS=30mas in
    # ≥1 band. That made DAO a SURVIVAL GATE and silently dropped 273K (35%)
    # real sources — incl. on-host SNe (468896/19931/39020) whose DAO peak
    # merges into the host light. Per the binding principle "DAO absence ≠
    # SN absence" (master CLAUDE.md §7.0/§8), KEEP ALL catalog sources inside
    # the JWST footprint. DAO photometry below becomes an ANNOTATION (filled
    # where matched, NaN otherwise), NOT a gate.
    import sys as _sys
    _sys.path.insert(0, '/Users/suzuki/github/projects_cosmos/programs_webpage')
    from tile_lookup import TileResolver
    _res = TileResolver()
    _grp = _res._idx.get(('jwst', 'F115W'))
    _ramin = _grp['ra_min'].values; _ramax = _grp['ra_max'].values
    _decmin = _grp['dec_min'].values; _decmax = _grp['dec_max'].values
    in_foot = np.zeros(len(cw_id), dtype=bool)
    for _i in range(len(_ramin)):
        in_foot |= ((cw_ra >= _ramin[_i]) & (cw_ra <= _ramax[_i]) &
                    (cw_dec >= _decmin[_i]) & (cw_dec <= _decmax[_i]))
    matched_cat_ids = np.where(in_foot)[0].astype(int)
    _dao_matched = set()
    for band in band_match:
        idx_c, _, _, _ = band_match[band]
        _dao_matched.update(idx_c.tolist())
    print(f'\n  v04: keeping ALL {len(matched_cat_ids):,} CW ids in JWST footprint '
          f'({len(_dao_matched):,} have ≥1 band DAO match; '
          f'{len(matched_cat_ids)-len(_dao_matched):,} DAO-absent kept '
          f'(on-host/faint/blended — NOT dropped))')
    ...
    # Attach per-band DAO photometry (NaN for DAO-absent rows = on-host/faint).
    cat_id_to_pos = {int(cid): i for i, cid in enumerate(matched_cat_ids)}
    for band in ('F115W', 'F150W', 'F277W', 'F444W'):
        idx_c, idx_d, sep, dao = band_match[band]
        # v04: a DAO match may point at a catalog source OUTSIDE the footprint
        # set (near edges). Keep only matches whose catalog id is in `out`.
        keep = np.array([int(ci) in cat_id_to_pos for ci in idx_c], dtype=bool)
        idx_c = np.asarray(idx_c)[keep]; idx_d = np.asarray(idx_d)[keep]
        sep = np.asarray(sep)[keep]
        pos = np.array([cat_id_to_pos[int(ci)] for ci in idx_c], dtype=int)
    ...
    p = OUT / 'cat_matched_JWST_v04.parquet'
    out.to_parquet(p, index=False)
    out.to_csv(p.with_suffix('.csv'), index=False)
...
def step3_euclid_vis():
    ...
    p = OUT / 'cat_matched_Euclid_VIS_v04.parquet'
    out.to_parquet(p, index=False)
    out.to_csv(p.with_suffix('.csv'), index=False)
    print(f'  → {p}  ({len(out):,} rows, {len(out.columns)} cols)')
...
def step3_euclid_nisp():
    ...
    p = OUT / 'cat_matched_Euclid_NISP_v04.parquet'
    out.to_parquet(p, index=False)
    out.to_csv(p.with_suffix('.csv'), index=False)
    print(f'  → {p}  ({len(out):,} rows, {len(out.columns)} cols)')
...
def main():
    import sys
    # v04 changes ONLY the JWST completeness gate. HST/VIS/NISP are unchanged
    # from v03 — by default regenerate JUST JWST (fast) and reuse the v03
    # outputs for the others. Pass --all to regenerate every survey as v04.
    if '--all' in sys.argv:
        step3_hst(); step3_jwst(); step3_euclid_vis(); step3_euclid_nisp()
    else:
        print("v04: regenerating JWST only (HST/VIS/NISP unchanged from v03)")
        step3_jwst()
    print('\nDone.')
