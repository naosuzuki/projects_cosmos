...
def process_combo(args) -> dict:
    ...
    df = pd.read_parquet(OUT / f'refined_{combo_name}_v03.parquet')
    n_total = len(df)
    ...
    out_parq = OUT / f'refined_{combo_name}_v03_with_pm.parquet'
    df.to_parquet(out_parq, index=False)
    csv_written = False
    if len(df) * len(df.columns) < 200_000_000:
        df.to_csv(OUT / f'refined_{combo_name}_v03_with_pm.csv', index=False)
        csv_written = True
