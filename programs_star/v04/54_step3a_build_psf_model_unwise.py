#!/usr/bin/env python
"""
54_step3a_build_psf_model_unwise.py — empirical PSF model for unWISE neo7
fulldepth coadds (W1/W2), porting the validated ground recipe (wide-Gaia
tilted stellar locus, purity limit + guards, 2.5xFWHM isolation, two-pass
PSFEx, per-tile saturation hook).

unWISE specifics (vs SDSS frames):
  * Tiles 2048x2048 @ 2.75"/px (1.56 deg); W1 PSF FWHM ~6" = 2.2 px —
    marginally sampled, the coarsest dataset in the stack.
  * MAGZP = 22.5 VEGA -> builder uses AB: ZP = 22.5 + 2.699 (W1) /
    + 3.339 (W2)  (Cutri+ 2013).
  * NO mask plane (img/invvar/n/std only): saturation via the counts
    ceiling visible in the data (~110k counts; local turnover for the
    pilot, unwise_global_onsets.json hook for production).
  * PSFVAR_DEGREES 1 (modest star counts per tile at WISE depth).

Outputs to /Volumes/exdisk1/data/photometry_v04/unwise_<band>/<tile>/psf/.
"""
...
UW_DIR = Path('/Volumes/exdisk1/data/unWISE/COSMOS/neo7')
WORK    = Path('/Volumes/exdisk1/data/photometry_v04')
PIXSCALE = 2.75
VEGA2AB  = {'w1': 2.699, 'w2': 3.339}   # Cutri+ 2013 WISE Vega->AB
VIG_HALF = 50
...
def parse_args():
    ...
    p.add_argument('--tile', default='1497p015',
                   help='unWISE coadd tile (default: contains COSMOS centre)')
    p.add_argument('--filter', default='w1', choices=['w1', 'w2'])
...
def tile_file(tile, band):
    f = UW_DIR / tile[:3] / tile / f'unwise-{tile}-{band}-img-m.fits'
    if not f.exists():
        sys.exit(f'No unWISE image for {tile} band {band}')
    return f


def main():
    a = parse_args()
    band, frame = a.filter, a.tile
    suffix = frame
    inst = f'unwise_{band}'
    out = WORK / inst / frame / 'psf'
    out.mkdir(parents=True, exist_ok=True)

    img_bz = tile_file(frame, band)
    with fits.open(img_bz) as h:
        sci   = h[0].data.astype(np.float32)     # counts (Vega zp 22.5)
        imhdr = h[0].header
    sci[~np.isfinite(sci)] = 0.0
    zp = 22.5 + VEGA2AB[band]                    # AB zeropoint
    img_clean = out / f'image_{suffix}.fits'
    fits.PrimaryHDU(sci, header=imhdr).writeto(img_clean, overwrite=True)
    sex_img = str(img_clean)

    print(f'Instrument : {inst}  (unWISE neo7 fulldepth coadd)')
    print(f'Tile/band  : {frame} / {band}   ZP_AB={zp:.3f} (=22.5 Vega +{VEGA2AB[band]})')
    ...
    gj = PROJECT / 'programs_star' / 'csv_saturation' / 'unwise_global_onsets.json'
    gv = (json.loads(gj.read_text()).get('bands', {}).get(band, {})
          if gj.exists() else {})
    onset_glob = gv.get('sat_onset_mag')
    K_dn = gv.get('ceiling_K_counts')
    if onset_glob is not None and K_dn is not None:
        C_tile = float(K_dn)
    ...
    print(f'  saturation : {int(saturated.sum())} excluded  '
          ...
          f'[{sat_onset_method}]'
          + (f'; ceiling {sat_peak_level:.4g} counts'
    ...
    if int(locus_base.sum()) < 10:
        (out / f'psf_{suffix}.meta.json').write_text(json.dumps({
            ...
            'channel': 'unwise', 'status': 'no_coverage',
    ...
    meta = {
        'created_utc_iso': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()),
        'instrument': inst, 'tile': frame, 'filter': band, 'channel': 'unwise',
