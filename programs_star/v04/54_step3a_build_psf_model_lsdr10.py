#!/usr/bin/env python
"""
54_step3a_build_psf_model_lsdr10.py — empirical PSF model for DESI Legacy
Surveys DR10 (DECam south) brick coadds, porting the HSC-validated ground
recipe (tilted magnitude-dependent stellar locus, measured saturation
magnitude, 2.5xFWHM isolation, two-pass PSFEx).

LS DR10 specifics (vs HSC deepCoadd):
  * Brick coadds 3600x3600 at 0.262"/px (~0.27 deg^2), fpack-compressed:
        legacysurvey-<brick>-image-<b>.fits.fz   [1] = science, nanomaggy
        legacysurvey-<brick>-maskbits.fits.fz    [1] = bit-packed mask
    (invvar exists but, as for HSC, WEIGHT NONE self-estimated noise is
    used — the validated ground recipe.)
  * ZP_AB = 22.5 EXACT (nanomaggy definition) — not provisional.
  * Saturation/bad pixels from maskbits (band-dependent bits):
        SATUR_G=2  SATUR_R=3  SATUR_I=14  SATUR_Z=4   -> stellar-core reject
        ALLMASK_G=5 ALLMASK_R=6 ALLMASK_I=15 ALLMASK_Z=7 -> core reject
        NPRIMARY=0                                     -> gap (off-brick area)
  * Seeing ~1.0-1.4" -> PSF FWHM ~4-5.5 px: the SAME pixel-sampling regime
    as HSC (0.168"/px, ~5 px FWHM), so configs/psfex_hsc.psfex is reused
    directly (SAMPLE_FWHMRANGE is overridden dynamically anyway).

Outputs to /Volumes/exdisk1/data/photometry_v04/lsdr10_<band>/<brick>/psf/.
"""
...
# maskbits bit assignments (from the maskbits header MBIT_* keywords)
SATUR_BIT   = {'g': 2, 'r': 3, 'i': 14, 'z': 4}
ALLMASK_BIT = {'g': 5, 'r': 6, 'i': 15, 'z': 7}
B_NPRIMARY  = 0
...
def parse_args():
    ...
    p.add_argument('--brick', default='1501p022',
                   help='LS DR10 brickname (default: COSMOS centre)')
    p.add_argument('--filter', default='i', choices=['g', 'r', 'i', 'z'])
    p.add_argument('--core-box', type=int, default=5)
    p.add_argument('--snr-min', type=float, default=8.0,
                   help='SNR_WIN floor for PSF stars (WEIGHT-NONE compressed '
                        'scale, as validated on HSC).')
    p.add_argument('--nbr-fwhm', type=float, default=2.5,
                   help='Isolation radius in PSF-FWHM units: reject a PSF star '
                        'if ANY detected object lies within (no brightness '
                        'gate).  2.5xFWHM = the model stamp.')
...
def brick_file(brick, ftype):
    f = LS_DIR / brick[:3] / brick / f'legacysurvey-{brick}-{ftype}.fits.fz'
    if not f.exists():
        sys.exit(f'No LS DR10 file: {f}')
    return f


def main():
    a = parse_args()
    band, brick = a.filter, a.brick
    suffix = brick          # output-file key = brick (filter is in the instrument dir)
    inst = f'lsdr10_{band}'
    out = WORK / inst / brick / 'psf'
    out.mkdir(parents=True, exist_ok=True)

    img_fz = brick_file(brick, f'image-{band}')
    msk_fz = brick_file(brick, 'maskbits')
    # SExtractor's handling of fpack tile compression is unreliable → extract a
    # clean single-HDU science image (astropy decompresses fine), keeping the
    # WCS header.  maskbits stays in-memory (pixel tests only).
    with fits.open(img_fz) as h:
        sci   = h[1].data.astype(np.float32)
        imhdr = h[1].header
    ...
    print(f'Brick/band : {brick} / {band.upper()}   ZP_AB={zp:.1f} (nanomaggy, exact)')
    print(f'Source     : {img_fz.name}')
    print(f'             extracted image[1]; WEIGHT NONE; maskbits saturation '
          f'(SATUR_{band.upper()}|ALLMASK_{band.upper()}); SNR>{a.snr_min}')
    ...
    # gaps (VIGNET over NPRIMARY) + neighbour contamination
    edge = (flg & 8) > 0
    ...
    # neighbour contamination: STRICT isolation within the model stamp scale
    ...
    # PSF model stamp SMALLER than the isolation radius → clean fit core
    ...
    # LS DR10 is SHALLOW (~23.5) with ~1.1-1.4" seeing: beyond ~21.5 the
    # CLASS_STAR>0.8 population is dominated by marginally-resolved compact
    # galaxies whose FLUX_RADIUS ridge sits just above the stellar locus, so a
    # point-source tilt fit (the HSC approach) latches onto galaxies.  Fit the
    # tilt on the GAIA-MATCHED stars only (same anchor philosophy as the locus
    # itself); robust fallback to the flat constant median.
    ...
    if anchored and int(tok.sum()) >= 20 and (np.ptp(tm_all[tok]) >= 1.5):
        ...
        slope = float(np.clip(slope_fit, -0.08, 0.02))  # sane ground slope px/mag
        if slope != slope_fit:
            # a clamped slope invalidates the JOINTLY-fit intercept (the line
            # would shift off the data); refit intercept + scatter at the
            # clamped slope.
    ...
    # The magnitude where the in-band point-source median FR departs from the
    # Gaia-anchored line by >2·MAD = where compact galaxies take over the band
    # (measured from the data, not an ad-hoc cut).  Selection stops there.
    ...
    print(f'  purity limit : '
          + (f'mag < {pure_to:.1f} (measured: in-band medFR departs locus by >2·MAD)'
             if pure_to is not None else 'none (band stays stellar to the SNR floor)'))
    # bright end = measured saturation mag, faint end = MEASURED galaxy-
    # contamination limit (+ S/N floor + the FLUX_RADIUS locus band)
    ...
    # ── 5. meta JSON (keys mirror the HSC builder for 56_single) ──
    meta = {
        'created_utc_iso': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()),
        'instrument': inst, 'tile': brick, 'filter': band, 'channel': 'lsdr10',
        ...
        'recipe': 'LS DR10 brick image[1] nanomaggy; WEIGHT NONE; maskbits '
                  'saturation; Gaia-anchored TILTED locus; two-pass PSF',
    }
    (out / f'psf_{suffix}.meta.json').write_text(json.dumps(meta, indent=2))
    print(f'\n[save] {out / f"psf_{suffix}.meta.json"}')
    print('=== LS DR10 PSF model build complete ===')
