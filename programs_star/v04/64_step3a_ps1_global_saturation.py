#!/usr/bin/env python
"""
64_step3a_ps1_global_saturation.py — GLOBAL per-band saturation onset for
Pan-STARRS1 rings.v3 stacks, pooled across skycells and anchored to Gaia DR3.
...
OUT_JSON = PROJECT / 'programs_star' / 'csv_saturation' / 'ps1_global_onsets.json'
GAIA_CSV = Path('/Volumes/exdisk1/data/catalog/Gaia/COSMOS/gaia_dr3_cosmos_wide.csv')


def all_cells():
    out = []
    for p in sorted(PS1_DIR.glob('*/*')):
        if p.is_dir():
            out.append(f'{p.parent.name}_{p.name}')
    return out


def cell_paths(cell, band):
    proj, sub = cell.split('_')
    base = PS1_DIR / proj / sub / f'rings.v3.skycell.{proj}.{sub}.stk.{band}.unconv'
    return Path(f'{base}.fits')
...
def ensure_pass1(cell, band):
    ...
    with fits.open(img_fz) as h:
        ...
        hd = h[1].header
    bs, bo = float(hd.get('BSOFTEN', np.nan)), float(hd.get('BOFFSET', np.nan))
    sci = (bo + bs * 2.0 * np.sinh(0.4 * np.log(10.0) * raw)
           if np.isfinite(bs) and np.isfinite(bo) else raw).astype(np.float32)
    sci[~np.isfinite(sci)] = 0.0
    exptime = float(hd.get('EXPTIME', 1.0))
...
def pooled_onset(band, min_cells, jobs=4, verbose=True):
    ...
    have = [c for c in cells
            if (WORK / f'ps1_{band}' / c / 'psf' / f'pass1_{c}.fits').exists()]
    ...
    for c, cat, _ in pre:
        ...
        try:
            ...
            with fits.open(img) as h:
                raw = h[1].data.astype(np.float64); hd = h[1].header
            bs, bo = float(hd.get('BSOFTEN', np.nan)), float(hd.get('BOFFSET', np.nan))
            sci = (bo + bs * 2.0 * np.sinh(0.4 * np.log(10.0) * raw)
                   if np.isfinite(bs) else raw).astype(np.float32)
            sci[~np.isfinite(sci)] = 0.0
        except Exception:
            continue
...
def main():
    ...
    ap.add_argument('--bands', default='g,r,i,z,y')
