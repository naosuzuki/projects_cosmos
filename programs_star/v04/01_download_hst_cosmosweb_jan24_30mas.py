#!/usr/bin/env python
"""
76_download_hst_cosmosweb_jan24_30mas.py — parallel download of the
COSMOS-Web Jan-2024 ACS/WFC F814W 30-mas mosaics.

Source : https://exchg.calet.org/cosmosweb/COSMOS-Web_Jan24/ACS/
Target : /Volumes/exdisk1/data/HST/COSMOS_ACS2024/
Auth   : HTTP Basic via ~/.netrc (already provisioned)

Files  : 10 tiles (B1..B10) × 3 products (drz, err, wht), 1.8 GB each
         → 30 files ≈ 54 GB total.  60-mas siblings are skipped.
...
Usage  :  python 76_download_hst_cosmosweb_jan24_30mas.py
          python 76_download_hst_cosmosweb_jan24_30mas.py --jobs 6
          python 76_download_hst_cosmosweb_jan24_30mas.py --dry-run
...
BASE_URL = 'https://exchg.calet.org/cosmosweb/COSMOS-Web_Jan24/ACS/'
DEST     = Path('/Volumes/exdisk1/data/HST/COSMOS_ACS2024')
TILES    = [f'B{i}' for i in range(1, 11)]
PRODUCTS = ['drz', 'err', 'wht']
PIXSCALE = '30mas'
APPROX_GB_PER_FILE = 1.8
...
def parse_args():
    ...
    p.add_argument('--jobs', '-j', type=int, default=2,
                   help='Number of files to download in parallel (default 2 — '
                        'server is bandwidth-capped; >2 triggers per-conn '
