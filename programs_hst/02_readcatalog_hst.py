...
hstdir='/Users/suzuki/data/HST/clutch/v0.2/'
hstpng='/Users/suzuki/data/HST/clutch/v0.2_png/'

hstdir='/Users/suzuki/data/HST/cosmosacs/original/'
hstpng='/Users/suzuki/data/HST/cosmosacs/original_png/'

def read_catalog(cosmoswebcsv,hstcosmosfitscsv):
   df=pd.read_csv(cosmoswebcsv)
   dffits=pd.read_csv(hstcosmosfitscsv,comments='#')
   print(df)
#   sys.exit(1)
...
#    print(pngname)
     ...
     for j in range(len(dffits)):
        ...
        print(j,flag)
        if(flag):
          [tilestring]=find_tilenumber_hst(fitsname_1)
          pngname=hstpng+cosmosid.zfill(6)+'_'+tilestring+'_hstcosmos.png'
 #        pngname_2=hstpng+cosmosid.zfill(6)+'_'+tilestring+'_hstclutch2.png'
...
#         for fitsfilename in [fitsname_1, fitsname_2, fitsname_3]:
          for fitsfilename in [fitsname_1]:
             count+=1
             hdul=fits.open(hstdir+fitsfilename)
             print(hstdir+fitsfilename)
             hdr=hdul[0].header
             wcs=WCS(hdr)
             # 6 arcsec cutout
...
#             print(cutout.data.shape)
             ...
             mag=magf150 ; magflag=0
             cutout_1=cutout.data
             create_blackwhite_hst(cutout_1,pngname,objtitle,z,mag,magflag,mass,age)
             del magflag
...
#               create_colorimg_hst(cutout_1,cutout_2,cutout_3,pngname_1,objtitle,z,mag,magflag,mass,age)
#             if(count==4):  
#               cutout_4=cutout.data ; del cutout ; magflag=1 ; mag=magf277
#               create_colorimg_hst(cutout_2,cutout_3,cutout_4,pngname_2,objtitle,z,mag,magflag,mass,age)
#               del magflag

#     print(i,ra,dec,z)

def find_tilenumber_hst(fitsfilename):
    tilestring=fitsfilename.split('_')[6]
    return [tilestring]

def check_fits(fitsfilename,ra,dec):
# Checking the coordinates are in FITS fiel
   print(hstdir+fitsfilename)
   f=fits.open(hstdir+fitsfilename)
   hdr=f[0].header   
   f.close()
   naxis1=hdr['NAXIS1']
   naxis2=hdr['NAXIS2']
...
def create_blackwhite_hst(data_1,pngname,objtitle,z,mag,magflag,mass,age):
...
# Replace negative value
   bw=numpy.where(bw>minimum,bw,minimum)
   bw=numpy.sqrt(bw)
   maximum = 0.001
   val=numpy.amax(bw[95:105,95:105])
   if(maximum<1.0): 
      maximum=1.0
   else:
      maximum*=0.85
...
#  plt.imshow(rgb, origin='lower')
   plt.imshow(bw, origin='lower', cmap='gray')
   plt.tight_layout()
...
#hstcosmosfitscsv='../csv/hstcosmosfits.csv'

hstcosmosfitscsv='../csv/hstcosmosfits.csv'
cosmoswebcsv='../csv/COSMOSWeb_masterv3.1_mag25cut.csv'
read_catalog(cosmoswebcsv,hstcosmosfitscsv)
