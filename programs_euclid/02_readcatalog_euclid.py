...
jwstzpt=28.0865 # COSMOS Web 30mas data
euclidpng='/Users/suzuki/data/Euclid/COSMOS_OTF/NISP_png/'
eucliddir='/Users/suzuki/data/Euclid/COSMOS_OTF/NISP/'

def read_catalog(cosmoswebcsv,euclidfitscsv):
   df=pd.read_csv(cosmoswebcsv)
   dffits=pd.read_csv(euclidfitscsv)
   print(df)
...
#    print(pngname)
     ...
     for j in range(len(dffits)):
        ...
        if(flag):
          pngname_1=euclidpng+cosmosid.zfill(6)+'_euclid.png'
          objtitle=cosmosid.zfill(6)

# For JWST
#         [tilestring]=find_tilenumber(fitsname_1)
#         pngname_1=euclidpng+cosmosid.zfill(6)+'_'+tilestring+'_jwst1.png'
#         pngname_2=euclidpng+cosmosid.zfill(6)+'_'+tilestring+'_jwst2.png'
...
# For Euclid
          for fitsfilename in [fitsname_1, fitsname_2, fitsname_3]:
             count+=1
             hdul=fits.open(eucliddir+fitsfilename)
             hdr=hdul[0].header
             zpt=hdr['MAGZERO']
#             print('zpt=',zpt)
#             sys.exit(1)
...
# For Euclid
             if((nx < 60) or (ny < 60)): continue
             if(count==1): cutout_1=cutout.data ; cutout_1*=10.0**(0.4*(jwstzpt-zpt)) ;  del cutout 
             if(count==2): cutout_2=cutout.data ; cutout_2*=10.0**(0.4*(jwstzpt-zpt)) ;  del cutout
             if(count==3):  
               cutout_3=cutout.data ; cutout_3*=10.0**(0.4*(jwstzpt-zpt)) ; del cutout ; magflag=0 ; mag=magf150
...
# Checking the coordinates are in FITS fiel
   f=fits.open(eucliddir+fitsfilename)
   hdr=f[0].header   
   f.close()
   naxis1=hdr['NAXIS1']
   naxis2=hdr['NAXIS2']
...
# Replace negative value
   ...
   for img in [b,g,r]:
      maximum = 0.001
# This is for Euclid (60 pix x 60 pix)
...
#   print(objtitle,'before min=',minimum,'max=',maximum)
   print('max check',maximum)
   if(maximum<1.0): 
      maximum=1.0
   else:
      #maximum*=0.95
      maximum*=1.2
   print('max check',maximum)
#   print(objtitle,'after min=',minimum,'max=',maximum)
...
#   plt.rcParams["font.family"]="Times New Roman"
   plt.axis('off')
# For Euclid
   xmin=0.0 ; dx=60. ; ymin=0.0 ; dy=60.
# For HST/JWST
#   xmin=1.0 ; dx=200. ; ymin=1.0 ; dy=200.
   plt.text(xmin+dx*0.07,ymin+0.10*dy,"%-9s"%(objtitle),color='white',fontsize=16)
   plt.text(xmin+dx*0.40,ymin+0.10*dy,"z="+"%5.3f"%(z),color='white',fontsize=16)
   if(magflag==0): 
...
euclidfitscsv='../csv/euclidfits.csv'
cosmoswebcsv='../csv/COSMOSWeb_masterv3.1_mag25cut.csv'
read_catalog(cosmoswebcsv,euclidfitscsv)
