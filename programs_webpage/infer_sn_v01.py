...
Architecture:
  ...
  - One watcher process runs alongside. Every WATCH_INTERVAL seconds it
    ...
    rule, and rewrites tbl_sn_candidates_v01.csv + the webpage.
...
Outputs:
  csvfiles_sn/_partial/infer_v01_<survey>.parquet      (worker checkpoints)
  csvfiles_sn/tbl_sn_candidates_v01.csv                (final + updated each cycle)
  csvfiles_sn/sn_candidates_v01_all_scored.parquet     (all sources, final)
  htmls/sn_search/v01/index.html                       (updated each watcher tick)
"""
...
def survey_worker(args):
    ...
    checkpoint_path = PART_DIR / f"infer_v01_{survey}.parquet"
...
def watcher(N_total, pid_arr, psrc_arr, ra_arr, dec_arr, th_arr, tj_arr, te_arr,
    ...
    update tbl_sn_candidates_v01.csv + webpage."""
    import time
    from pathlib import Path
    import numpy as np
    import pyarrow.parquet as pq
...
def update_outputs(N, pid, psrc, ra, dec, th, tj, te, in_h, in_j, in_e, thresholds, final=False):
    ...
    for survey in ("hst","jwst","vis","nisp"):
        cp = PART_DIR / f"infer_v01_{survey}.parquet"
    ...
    is_sn = (n_det == 1) & evaluated & (n_cov >= 2)  # require at least 2 surveys with coverage
