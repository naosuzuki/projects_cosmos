...
OUT_PT      = CSV_DIR / "cnn_v06.pt"
OUT_REPORT  = CSV_DIR / "train_v06_report.txt"
CUTOUT_CACHE= Path("/Volumes/My Book/data/cosmos_v06/_v06_cutouts_sigma.npz")  # σ-units cache (external disk)
...
MODEL_CH = 2 * N_CH         # CNN input channels: to_input() emits 10 (σ-rep + compactness-rep)
N_HOSTS = 3000          # galaxy hosts for injection + paired negatives
RNG_SEED = 606
...
def worker_extract(args):
    """One (survey, band, tile) FITS → all requested cutouts (stretched)."""
    import warnings; warnings.filterwarnings("ignore")
    import sys
    sys.path.insert(0, "/Users/suzuki/github/projects_cosmos/programs_webpage")
    import recut_3_hst as R_
    from astropy.io import fits
    from astropy.wcs import WCS
    import numpy as np
    survey, band, tile, reqs = args
    path = resolve(survey, band, tile)
    out = {}
...
def gather_cutouts(positions):
    ...
    Logs per-tile-group progress so an external-disk read hang is visible."""
    by_tile = defaultdict(list)
    for key, survey, band, tile, ra, dec in positions:
        by_tile[(survey, band, tile)].append((key, ra, dec))
    jobs = [(s,b,t,lst) for (s,b,t),lst in by_tile.items()]
    log(f"  {len(jobs)} (survey,band,tile) FITS groups to read ({N_WORKERS} workers)")
...
def make_sample(host, kind, rng, prof_j, prof_h):
    ...
    if kind == "pos_jwst":
        ...
        amp *= rng.uniform(0.7,1.4); sig *= rng.uniform(0.85,1.15)
        n=CUT; c=n//2
        x=c+int(rng.integers(-8,9)); y=c+int(rng.integers(-8,9))
        yy,xx=np.mgrid[0:n,0:n]
        for ci in (1,2,3,4):
...
def train_fold(X, y, device, epochs=25, batch=256, lr=1e-3, seed=0):
...
def ensemble_score(X, y, device, sn_tensor, K=7, epochs=25):
...
def loo_validate():
    ...
    sn_t = {sid: torch.tensor(to_input(sn_stacks[sid][0])[None], dtype=torch.float32, device=device)
            for sid in sids}
    K_ENS = 7
    log(f"LOO with {K_ENS}-model ensemble per fold (averaged scores)")
    scores = {}
    t_loo = time.time()
    for held in sids:
        # real positives = all POSITIVE-eligible SNe except the held-out one
        others = [real_aug[s] for s in pos_sids if s != held]
        Xr = np.concatenate(others, axis=0)
        yr = np.ones(len(Xr), dtype=np.float32)
        X = np.concatenate([Xs, Xr], axis=0)
        y = np.concatenate([ys, yr], axis=0)
        p, ps = ensemble_score(X, y, device, sn_t[held], K=K_ENS)
        scores[held] = (p, sn_stacks[held][1])
        rec_so_far = sum(1 for v,_ in scores.values() if v>=0.5)
        log(f"  held {held:>8s} ({sn_stacks[held][1]:>6s})  P={p:.4f} "
            f"(range {min(ps):.2f}-{max(ps):.2f})  [{rec_so_far}/{len(scores)} so far]")
    log(f"  LOO done in {time.time()-t_loo:.1f}s")
